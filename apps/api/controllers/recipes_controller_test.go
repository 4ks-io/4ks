...
type stubImageGenService struct {
	generateImageFn func(context.Context, string) ([]byte, error)
}

func (s stubImageGenService) GenerateImage(ctx context.Context, prompt string) ([]byte, error) {
	if s.generateImageFn != nil {
		return s.generateImageFn(ctx, prompt)
	}
	return []byte("png"), nil
}
...
func TestRecipeControllerGenerateRecipeAIImage(t *testing.T) {
	t.Parallel()

	t.Run("reserves media before generating and writes generated bytes", func(t *testing.T) {
		t.Parallel()
		...
		controller := NewRecipeController(
			...
			stubRecipeService{
				...
				reserveRecipeAIImageMediaFn: func(_ context.Context, recipeID string, userID string) (*models.RecipeMedia, error) {
					if recipeID != "recipe-1" || userID != "user-1" {
						t.Fatalf("unexpected reserve ids: %q %q", recipeID, userID)
					}
					return &models.RecipeMedia{ID: "ai-recipe-1", RecipeID: recipeID, Source: models.MediaSourceAI}, nil
				},
				writeRecipeMediaBytesFn: func(_ context.Context, media *models.RecipeMedia, data []byte) error {
					wrote = true
					if media.ID != "ai-recipe-1" || string(data) != "png-bytes" {
						t.Fatalf("unexpected write inputs: %+v %q", media, string(data))
					}
					return nil
				},
			},
			...
			stubFetcherService{},
			stubImageGenService{
				generateImageFn: func(_ context.Context, prompt string) ([]byte, error) {
					generated = true
					if prompt != "A delicious recipe photo for Soup" {
		...
		rec := performRecipeControllerRequest(t, controller.GenerateRecipeAIImage, http.MethodPost, "/api/recipes/recipe-1/ai-image", nil, func(ctx *gin.Context) {
			ctx.Params = gin.Params{{Key: "id", Value: "recipe-1"}}
			ctx.Set("id", "user-1")
		})
		...
		if !generated || !wrote {
			t.Fatal("expected image generation and media write")
		}
	})

	t.Run("existing ai media returns conflict before image generation", func(t *testing.T) {
		t.Parallel()
		...
		controller := NewRecipeController(
			...
			stubRecipeService{
				...
				reserveRecipeAIImageMediaFn: func(context.Context, string, string) (*models.RecipeMedia, error) {
					return nil, recipesvc.ErrRecipeAIImageAlreadyExists
				},
			},
			stubSearchService{},
			stubStaticService{},
		...
		if generated {
			t.Fatal("image generation should not run after reservation conflict")
		}
	})

	t.Run("generation failure marks reserved media errored", func(t *testing.T) {
		t.Parallel()

		var markedStatus models.MediaStatus
		controller := NewRecipeController(
			stubUserService{
				getUserByIDFn: func(context.Context, string) (*models.User, error) {
					return &models.User{ID: "user-1"}, nil
			...
			stubRecipeService{
				...
				reserveRecipeAIImageMediaFn: func(context.Context, string, string) (*models.RecipeMedia, error) {
					return &models.RecipeMedia{ID: "ai-recipe-1"}, nil
				},
				updateRecipeMediaStatusFn: func(_ context.Context, mediaID string, status models.MediaStatus) error {
					if mediaID != "ai-recipe-1" {
		...
		if markedStatus != models.MediaStatusErrorUnknown {
			t.Fatalf("expected error status, got %v", markedStatus)
		}
	})
}
